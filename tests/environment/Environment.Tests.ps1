...
Assert 'desktop Ready smoke implements complete Supplier Invoice Posting workflow' {
 ...
 foreach($phase in @(@('ConfirmPostSupplierInvoiceButton','SupplierInvoicePosting: post PASS'),@('Supplier Invoice posting re-entry failed.','SupplierInvoicePosting: re-entry PASS'))){$start=$fn.IndexOf($phase[0],[StringComparison]::Ordinal);$end=$fn.IndexOf($phase[1],$start,[StringComparison]::Ordinal);if($start-lt0-or$end-lt$start){throw "posting oracle phase missing: $($phase[0])"};$body=$fn.Substring($start,$end-$start);foreach($required in 'Get-SupplierInvoicesReadinessState','LogicalRows.Count','LogicalRowNames.Count','LogicalRowNames','expectedInvoiceRow','-ceq','Zaksięgowana','SupplierInvoicesTotal'){if(-not$body.Contains($required)){throw "posting exact row oracle missing $required in $($phase[0])"}}}

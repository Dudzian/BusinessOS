...
function Invoke-SupplierInvoicePostingSmoke($Main) {
    ...
    Invoke-SupplierInvoicesPhase $Main 'posting dialog' $project {$row=Get-SupplierInvoiceRowState $Main 'Smoke Equipment Vendor' 'SMOKE-INV-001' 135 'PLN' '2026-01-10' '2026-02-10';Select-ContainingListItem $row.ListItem;Wait-BusinessOSCondition -TimeoutSeconds 10 -RequiredConsecutiveSuccesses 3 -TimeoutMessage 'Posting action not ready.' -Condition{try{(Get-AutomationIdElement $Main 'PostSupplierInvoiceButton').Current.IsEnabled-and(Get-AutomationIdElement $Main 'EditSupplierInvoiceButton').Current.IsEnabled-and(Get-AutomationIdElement $Main 'ArchiveSupplierInvoiceButton').Current.IsEnabled}catch{$false}};Invoke-AutomationIdButton $Main 'PostSupplierInvoiceButton';Wait-BusinessOSCondition -TimeoutSeconds 10 -RequiredConsecutiveSuccesses 3 -TimeoutMessage 'Posting dialog not ready.' -Condition{try{$dialog=Get-AutomationIdElement $Main 'PostSupplierInvoiceDialog';$null-ne$dialog-and(Get-AutomationIdElement $dialog 'PostSupplierInvoiceSourceSupplier').Current.Name-eq'Smoke Equipment Vendor'-and(Get-AutomationIdElement $dialog 'PostSupplierInvoiceSourceNumber').Current.Name-eq'SMOKE-INV-001'-and(Get-AutomationIdElement $dialog 'PostSupplierInvoiceSourceAmount').Current.Name-match'135 PLN'-and(Get-AutomationIdElement $dialog 'PostSupplierInvoiceSourceInvoiceDate').Current.Name-eq'2026-01-10'-and(Get-AutomationIdElement $dialog 'PostSupplierInvoiceKindSelector').Current.IsEnabled-and-not(Get-AutomationIdElement $dialog 'ConfirmPostSupplierInvoiceButton').Current.IsEnabled-and-not(Get-AutomationIdElement $Main 'CompaniesSectionButton').Current.IsEnabled-and-not(Get-AutomationIdElement $Main 'OpenRecoveryFromMainButton').Current.IsEnabled}catch{$false}}};Add-Content $diagnostics 'SupplierInvoicePosting: dialog PASS'
    Select-ComboBoxExactSemanticItem $Main 'PostSupplierInvoiceKindSelector' 'Capex';Wait-BusinessOSCondition -TimeoutSeconds 10 -RequiredConsecutiveSuccesses 3 -TimeoutMessage 'Posting kind did not stabilize.' -Condition{try{(Get-ComboBoxSemanticSelection (Get-AutomationIdElement $Main 'PostSupplierInvoiceKindSelector') 'Capex').IsExpected-and(Get-AutomationIdElement $Main 'ConfirmPostSupplierInvoiceButton').Current.IsEnabled}catch{$false}};Invoke-AutomationIdButton $Main 'ConfirmPostSupplierInvoiceButton';Wait-BusinessOSCondition -TimeoutSeconds 20 -RequiredConsecutiveSuccesses 3 -TimeoutMessage 'Invoice posting failed.' -Condition{try{$row=Get-SupplierInvoiceRowState $Main 'Smoke Equipment Vendor' 'SMOKE-INV-001' 135 'PLN' '2026-01-10' '2026-02-10';$status=Get-NamedElement $row.ListItem 'Zaksięgowana';$state=Get-SupplierInvoicesReadinessState $Main $project;$null-eq(Get-AutomationIdElement $Main 'PostSupplierInvoiceDialog')-and$state.LogicalRows.Count-eq1-and$state.LogicalRowNames.Count-eq1-and($state.LogicalRowNames-join'')-ceq$expectedInvoiceRow-and$null-ne$status-and-not(Get-AutomationIdElement $Main 'PostSupplierInvoiceButton').Current.IsEnabled-and-not(Get-AutomationIdElement $Main 'EditSupplierInvoiceButton').Current.IsEnabled-and-not(Get-AutomationIdElement $Main 'ArchiveSupplierInvoiceButton').Current.IsEnabled-and(Get-AutomationIdElement $Main 'SupplierInvoicesTotal').Current.Name-eq'Total: 135'-and(Get-AutomationIdElement $Main 'CompaniesSectionButton').Current.IsEnabled-and(Get-AutomationIdElement $Main 'OpenRecoveryFromMainButton').Current.IsEnabled}catch{$false}};Add-Content $diagnostics 'SupplierInvoicePosting: post PASS'
    Invoke-AutomationIdButton $Main 'ActualCostsSectionButton';Wait-BusinessOSCondition -TimeoutSeconds 20 -RequiredConsecutiveSuccesses 3 -TimeoutMessage 'Actual Costs posting selector not ready.' -Condition{try{(Get-AutomationIdElement $Main 'ActualCostsProjectSelector').Current.IsEnabled}catch{$false}};Select-ComboBoxExactSemanticItem $Main 'ActualCostsProjectSelector' $project;Wait-BusinessOSCondition -TimeoutSeconds 20 -RequiredConsecutiveSuccesses 3 -TimeoutMessage 'Posted Actual Cost missing.' -Condition{try{(Get-ActualCostRowState $Main 'Faktura SMOKE-INV-001' 135 'Capex').Matches-and(Get-AutomationIdElement $Main 'ActualCostCapexTotal').Current.Name-eq'CAPEX: 285'-and(Get-AutomationIdElement $Main 'ActualCostOpexTotal').Current.Name-eq'OPEX: 0'-and(Get-AutomationIdElement $Main 'ActualCostTotal').Current.Name-eq'Total: 285'}catch{$false}};Add-Content $diagnostics 'SupplierInvoicePosting: actual cost PASS'
    Invoke-AutomationIdButton $Main 'CompaniesSectionButton';Wait-BusinessOSCondition -TimeoutSeconds 10 -RequiredConsecutiveSuccesses 3 -TimeoutMessage 'Supplier Invoices posting re-entry navigation not ready.' -Condition{try{(Get-AutomationIdElement $Main 'SupplierInvoicesSectionButton').Current.IsEnabled}catch{$false}};Invoke-AutomationIdButton $Main 'SupplierInvoicesSectionButton';Wait-BusinessOSCondition -TimeoutSeconds 20 -RequiredConsecutiveSuccesses 3 -TimeoutMessage 'Supplier Invoices posting re-entry selector not ready.' -Condition{try{(Get-AutomationIdElement $Main 'SupplierInvoicesProjectSelector').Current.IsEnabled}catch{$false}};Select-ComboBoxExactSemanticItem $Main 'SupplierInvoicesProjectSelector' $project;Wait-BusinessOSCondition -TimeoutSeconds 20 -RequiredConsecutiveSuccesses 3 -TimeoutMessage 'Supplier Invoice posting re-entry failed.' -Condition{try{$row=Get-SupplierInvoiceRowState $Main 'Smoke Equipment Vendor' 'SMOKE-INV-001' 135 'PLN' '2026-01-10' '2026-02-10';Select-ContainingListItem $row.ListItem;$state=Get-SupplierInvoicesReadinessState $Main $project;$state.LogicalRows.Count-eq1-and$state.LogicalRowNames.Count-eq1-and($state.LogicalRowNames-join'')-ceq$expectedInvoiceRow-and$null-ne(Get-NamedElement $row.ListItem 'Zaksięgowana')-and-not(Get-AutomationIdElement $Main 'PostSupplierInvoiceButton').Current.IsEnabled-and-not(Get-AutomationIdElement $Main 'EditSupplierInvoiceButton').Current.IsEnabled-and-not(Get-AutomationIdElement $Main 'ArchiveSupplierInvoiceButton').Current.IsEnabled-and(Get-AutomationIdElement $Main 'SupplierInvoicesTotal').Current.Name-eq'Total: 135'}catch{$false}};Add-Content $diagnostics 'SupplierInvoicePosting: re-entry PASS'
}

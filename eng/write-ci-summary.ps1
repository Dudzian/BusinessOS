...
$identity=Get-BusinessOSGitHubIdentity -CheckoutSha $checkoutSha
...
if(-not$MinimalFailure){
 ...
 $scenarioFiles=@(Get-ChildItem (Join-Path $root 'artifacts/smoke-test/scenarios') -File -Filter *.json -ErrorAction SilentlyContinue);$scenarios=[object[]]@($scenarioFiles|ForEach-Object{Get-Content $_.FullName -Raw|ConvertFrom-Json -Depth 20})
 $passedScenarioCount=@($scenarios|Where-Object{$_.status-eq'PASS'}).Count
 $failedScenarioCount=@($scenarios|Where-Object{$_.status-ne'PASS'}).Count

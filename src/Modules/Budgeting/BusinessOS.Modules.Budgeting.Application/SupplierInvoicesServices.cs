...
public enum SupplierInvoiceOperationStatus { Success, ValidationFailure, NotFound, ConcurrencyConflict, ProjectUnavailable, Archived, DuplicateInvoice, PersistenceFailure, Cancelled }
...
public sealed record SupplierInvoiceItem(Guid Id, Guid ProjectId, string SupplierName, string InvoiceNumber, decimal Amount, string Currency, DateOnly InvoiceDate, DateOnly DueDate, string? Note, DateTimeOffset CreatedAtUtc, DateTimeOffset UpdatedAtUtc, long Version)
{
...
internal sealed class SupplierInvoicesCrudService(ISupplierInvoicesStore store, IBudgetingProjectLookup projects, TimeProvider clock) : ISupplierInvoicesCrudService
{
    ...
    public Task<SupplierInvoiceResult<SupplierInvoiceItem>> UpdateAsync(Guid id, long version, string supplier, string number, decimal amount, string currency, DateOnly invoiceDate, DateOnly dueDate, string? note, CancellationToken ct = default) => Guard(async () =>
    {
        ...
        if (invoice.ArchivedAtUtc is not null) return Fail(SupplierInvoiceOperationStatus.Archived, "Faktura jest zarchiwizowana."); if (invoice.Version != version) return Fail(SupplierInvoiceOperationStatus.ConcurrencyConflict, "Faktura została zmieniona.");
    ...
    {
        var r = await Guard(async () => { var x = await store.GetAsync(new(id), true, ct); if (x is null) return Fail(SupplierInvoiceOperationStatus.NotFound, "Nie znaleziono faktury."); if (x.ArchivedAtUtc is not null) return Fail(SupplierInvoiceOperationStatus.Archived, "Faktura jest zarchiwizowana."); if (x.Version != version) return Fail(SupplierInvoiceOperationStatus.ConcurrencyConflict, "Faktura została zmieniona."); if (await projects.GetAsync(x.ProjectId.Value, ct) is not { Available: true }) return Fail(SupplierInvoiceOperationStatus.ProjectUnavailable, "Projekt nie jest dostępny."); x.Archive(clock.GetUtcNow()); return await Save(x, "Faktura została zarchiwizowana.", ct); }, ct); return new(r.Status, r.SafeMessage);
    }
    ...
    private static SupplierInvoiceItem Map(SupplierInvoice x) => new(x.Id.Value, x.ProjectId.Value, x.SupplierName, x.InvoiceNumber, x.Amount.Amount, x.Amount.Currency.Value, x.InvoiceDate, x.DueDate, x.Note, x.CreatedAtUtc, x.UpdatedAtUtc, x.Version);
}

...
public sealed class ActualCostsMigrationTests : IDisposable
{
    ...
    public async Task Fresh_database_applies_all_budgeting_migrations_and_creates_cost_indexes()
    {
        ...
        (await Strings("SELECT MigrationId FROM __EFMigrationsHistory_Budgeting ORDER BY MigrationId")).Should().Equal("20260808231124_InitialBudgetingPersistence", "20260810122833_AddActualCosts", "20260811102427_AddForecastCosts");
    ...
    public async Task Three_b_one_data_is_preserved_when_actual_costs_migration_is_applied()
    {
        ...
        await db.Database.MigrateAsync();
        (await Scalar("SELECT COUNT(*) FROM budgets")).Should().Be(1); (await Scalar("SELECT COUNT(*) FROM budget_versions")).Should().Be(1); (await Scalar("SELECT COUNT(*) FROM budget_lines")).Should().Be(1); (await Scalar("SELECT COUNT(*) FROM actual_costs")).Should().Be(0); (await Scalar("SELECT COUNT(*) FROM __EFMigrationsHistory_Budgeting")).Should().Be(3);
    }

...
Assert 'cross-platform filter contains required projects and excludes deferred projects' { $p=Get-BusinessOSCrossPlatformFilterProjects $RepoRoot; foreach($r in 'BusinessOS.BuildingBlocks.Domain.csproj','BusinessOS.BuildingBlocks.Application.csproj','BusinessOS.Modules.Companies.Infrastructure.csproj','BusinessOS.UnitTests.csproj','BusinessOS.ArchitectureTests.csproj','BusinessOS.IntegrationTests.csproj','BusinessOS.MigrationTests.csproj'){if(-not(($p|Split-Path -Leaf)-contains $r)){throw "missing $r"}}; if(($p -join ';') -match 'Desktop|AppHost|BusinessProjects.Infrastructure|BuildingBlocks.Infrastructure'){throw 'deferred project present'} }

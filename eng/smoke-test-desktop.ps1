...
function Test-InvariantLabeledDecimal([string]$Text,[string]$Label,[decimal]$Expected) {
    $pattern='^'+[regex]::Escape($Label)+': (?<Amount>[0-9]+(?:\.[0-9]+)?)$'
    if($Text-cnotmatch$pattern){return $false}
    [decimal]$amount = 0
    if(-not[decimal]::TryParse($Matches.Amount,[System.Globalization.NumberStyles]::AllowDecimalPoint,[System.Globalization.CultureInfo]::InvariantCulture,[ref]$amount)){return $false}
    $amount-eq$Expected
}

...
Assert 'desktop Ready smoke implements complete Supplier Invoice Posting workflow' {
 ...
 foreach($required in 'LogicalRows.Count','LogicalRowNames.Count','LogicalRowNames','ExpectedInvoiceRow','-ceq','Zaksięgowana','PostButtonFound','EditButtonFound','ArchiveButtonFound','Total: 135','Faktura została zaksięgowana jako koszt.','CompaniesButtonEnabled','RecoveryButtonEnabled','DialogMarkerFound','DialogMarkerName','DialogMarkerIsOffscreen'){if(-not$postState.Contains($required)){throw "posting post-state helper missing $required"}}

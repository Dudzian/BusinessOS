...
try{
 ...
 $event="$root/.cache/test-pr-event.json";$pr=$base.Clone();$pr.GITHUB_EVENT_NAME='pull_request';$pr.GITHUB_EVENT_PATH=$event
 foreach($case in @(@{Name='null merge_commit_sha';Payload=@{number=42;pull_request=@{head=@{sha='a'*40};merge_commit_sha=$null}}},@{Name='missing merge_commit_sha';Payload=@{number=42;pull_request=@{head=@{sha='a'*40}}}},@{Name='payload-vs-checkout SHA';Payload=@{number=42;pull_request=@{head=@{sha='a'*40};merge_commit_sha='b'*40}}})){
  $case.Payload|ConvertTo-Json -Depth 5|Set-Content $event;Set-Env $pr;if((Invoke-Writer -Minimal)-ne 0){throw "PR Actions generator failed for $($case.Name)"};$x=Read-Generated
  if($x.checkoutKind-ne'pull-request-merge'-or$x.runId-isnot[long]-or$x.runAttempt-isnot[long]-or$x.pullRequestNumber-ne 42-or$x.pullRequestHeadSha-ne('a'*40)-or$x.pullRequestMergeSha-ne$head-or$x.commitSha-ne$head){throw "PR identity was invalid for $($case.Name)"};Write-Host "Generator PR Actions $($case.Name) PASS"
 }
 Write-Host 'Generator PR Actions numeric identity PASS'
 ...
 @{number=77;pull_request=@{head=@{sha='c'*40};merge_commit_sha=$null}}|ConvertTo-Json -Depth 5|Set-Content $event;Set-Env $pr
 $fallbackPath=Write-BusinessOSCiEvidenceFallback -Gate cross-platform -Root $root -FailureStage 'build' -FailureMessage 'original gate failure' -GeneratorError 'controlled generator failure' -StartedAtUtc ([DateTimeOffset]::UtcNow.AddSeconds(-1).ToString('o'))
 $fallbackRaw=Get-Content $fallbackPath -Raw;$fallback=$fallbackRaw|ConvertFrom-Json -Depth 30;Test-BusinessOSCiEvidence $fallback|Out-Null
 if(-not($fallbackRaw|Test-Json -SchemaFile $schema -ErrorAction SilentlyContinue)){throw 'Fallback PR evidence failed JSON Schema'}
 if($fallback.gateStatus-ne'FAIL'-or$fallback.checkoutKind-ne'pull-request-merge'-or$fallback.pullRequestNumber-ne 77-or$fallback.pullRequestHeadSha-ne('c'*40)-or$fallback.pullRequestMergeSha-ne$head-or$fallback.runId-isnot[long]-or$fallback.runAttempt-isnot[long]-or$fallback.failure.stage-ne'build'-or$fallback.failure.message-ne'original gate failure'-or@($fallback.warnings|Where-Object{$_-like'*controlled generator failure*'}).Count-ne 1){throw 'Fallback PR identity or original gate failure was invalid'}
 Write-Host 'Exported fallback PR identity with null merge_commit_sha PASS'
 ...
 $projectsReadiness=[regex]::Match($smokeSource,'(?s)function Get-BusinessProjectsReadinessState\(\$Main, \[string\]\$ExpectedCompany\) \{.*?^\}',[Text.RegularExpressions.RegexOptions]::Multiline).Value
 if(-not$projectsReadiness-or$projectsReadiness-notmatch"(?s)BusinessProjectsCompanySelector.*?Get-ComboBoxSemanticSelection \`$selector \`$ExpectedCompany.*?\`$semanticSelection\.IsExpected"){throw 'Desktop smoke does not use the read-only semantic result for the BusinessProjects company selector'}
 if($smokeSource-notmatch"Get-BusinessProjectsReadinessState \`$Main 'BusinessOS Smoke Updated'\)\.IsReady"){throw 'Desktop smoke does not require the expected company in BusinessProjects readiness'}

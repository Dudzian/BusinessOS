...
Assert 'desktop Ready smoke protects the complete semantic Budgeting workflow' {
 ...
 $v2Marker=$body.IndexOf("TimeoutMessage 'v2 reselection'",[StringComparison]::Ordinal);if($v2Marker-lt0){throw 'missing V2 reselection timeout marker'};$v2Start=$body.LastIndexOf('Wait-BusinessOSCondition',$v2Marker,[StringComparison]::Ordinal);$v2End=$body.IndexOf('BudgetingCrud: revision isolation PASS',$v2Marker,[StringComparison]::Ordinal);if($v2Start-lt0-or$v2End-lt$v2Start){throw 'missing bounded V2 reselection contract'};$v2ReselectionContract=$body.Substring($v2Start,$v2End-$v2Start)
 foreach($required in 'Get-ComboBoxSemanticSelection','BudgetVersionsList','Version 2','.IsExpected','RequiredConsecutiveSuccesses 3','Get-BudgetLineRowState','Smoke CAPEX','150','Smoke Revenue','250','BudgetCapexTotal','BudgetRevenueTotal'){if(-not$v2ReselectionContract.Contains($required)){throw "V2 reselection contract missing: $required"}}
 if($v2ReselectionContract-match "(?s)Get-NamedElements.*?'Version 2'\s*\)\s*\.Count\s*-?eq\s*1"){throw 'V2 reselection must not use raw UIA descendant uniqueness as a correctness gate'}
...
Assert 'desktop Ready smoke implements complete semantic Actual Costs workflow' {
 $path=Join-Path $RepoRoot 'eng/smoke-test-desktop.ps1';$tokens=$null;$errors=$null;$ast=[System.Management.Automation.Language.Parser]::ParseFile($path,[ref]$tokens,[ref]$errors);if($errors.Count){throw 'desktop smoke does not parse'}
 foreach($name in 'Get-ActualCostsReadinessState','Get-ActualCostRowState','Test-ActualCostEditorReady','Test-ActualCostEditorClosed','Write-ActualCostsTimeoutDiagnostics','Invoke-ActualCostsPhase','Invoke-ActualCostsCrudSmoke'){if(@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true)).Count-ne1){throw "missing unique $name"}}
 $fn=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Invoke-ActualCostsCrudSmoke'},$true))[0].Extent.Text
 foreach($required in 'BusinessOS Gym Smoke Updated','Select-ComboBoxExactSemanticItem','Get-ActualCostRowState','RequiredConsecutiveSuccesses 3','Smoke Fit-out','Smoke Rent','ActualCostsCrud: readiness PASS','ActualCostsCrud: create PASS','ActualCostsCrud: totals PASS','ActualCostsCrud: update PASS','ActualCostsCrud: re-entry PASS','ActualCostsCrud: archive cancel PASS','ActualCostsCrud: archive PASS'){if(-not$fn.Contains($required)){throw "Actual Costs smoke missing: $required"}}
 foreach($forbidden in 'Select-Object -First','Select-Object -Last','Start-Sleep','IsOffscreen','BoundingRectangle','Test-Visible','[0]'){if($fn.Contains($forbidden)){throw "forbidden Actual Costs smoke construct: $forbidden"}}
 foreach($helperName in 'Test-ActualCostEditorReady','Test-ActualCostEditorClosed','Get-ActualCostsReadinessState'){$helper=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$helperName},$true))[0].Extent.Text;foreach($forbidden in 'IsOffscreen','BoundingRectangle','Test-Visible'){if($helper.Contains($forbidden)){throw "$helperName contains forbidden correctness dependency: $forbidden"}}}
 foreach($helperName in 'Test-ActualCostEditorReady','Test-ActualCostEditorClosed'){$helper=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$helperName},$true))[0].Extent.Text;foreach($required in 'try','catch','$false'){if(-not$helper.Contains($required)){throw "$helperName is not transient-exception safe"}}}
 $firstStart=$fn.IndexOf("'create fit-out'",[StringComparison]::Ordinal);$firstAdd=$fn.IndexOf("'AddActualCostButton'",$firstStart,[StringComparison]::Ordinal);$firstEditor=$fn.IndexOf('Test-ActualCostEditorReady',$firstAdd,[StringComparison]::Ordinal);$firstSave=$fn.IndexOf("'SaveActualCostButton'",$firstEditor,[StringComparison]::Ordinal);$firstPost=$fn.IndexOf('Fit-out post-save semantic wait failed.',$firstSave,[StringComparison]::Ordinal);$secondStart=$fn.IndexOf("'create rent'",$firstPost,[StringComparison]::Ordinal);$secondAdd=$fn.IndexOf("'AddActualCostButton'",$secondStart,[StringComparison]::Ordinal);if($firstStart-lt0-or$firstAdd-lt$firstStart-or$firstEditor-lt$firstAdd-or$firstSave-lt$firstEditor-or$firstPost-lt$firstSave-or$secondAdd-lt$firstPost){throw 'first save to second add sequencing contract is invalid'}
 $reentryStart=$fn.IndexOf("'re-entry'",[StringComparison]::Ordinal);$companies=$fn.IndexOf("'CompaniesSectionButton'",$reentryStart,[StringComparison]::Ordinal);$actual=$fn.IndexOf("'ActualCostsSectionButton'",$companies,[StringComparison]::Ordinal);$selectorReady=$fn.IndexOf('Actual Costs re-entry selector not ready.',$actual,[StringComparison]::Ordinal);$semanticSelect=$fn.IndexOf('Select-ComboBoxExactSemanticItem',$selectorReady,[StringComparison]::Ordinal);$reentryPass=$fn.IndexOf('ActualCostsCrud: re-entry PASS',$semanticSelect,[StringComparison]::Ordinal);if($reentryStart-lt0-or$companies-lt$reentryStart-or$actual-lt$companies-or$selectorReady-lt$actual-or$semanticSelect-lt$selectorReady-or$reentryPass-lt$semanticSelect){throw 'Actual Costs re-entry sequencing contract is invalid'}
 $editStart=$fn.IndexOf("'update'",[StringComparison]::Ordinal);$fitOut=$fn.IndexOf("'Smoke Fit-out'",$editStart,[StringComparison]::Ordinal);$editReady=$fn.IndexOf('Edit Actual Cost selection did not stabilize.',$fitOut,[StringComparison]::Ordinal);$editClick=$fn.IndexOf("Invoke-AutomationIdButton `$Main 'EditActualCostButton'",$editReady,[StringComparison]::Ordinal);$editorReady=$fn.IndexOf('Edit Actual Cost editor readiness failed.',$editClick,[StringComparison]::Ordinal);$amount150=$fn.IndexOf("Set-AutomationValue `$Main 'ActualCostAmountInput' '150'",$editorReady,[StringComparison]::Ordinal);$updatePost=$fn.IndexOf('Actual Costs update failed.',$amount150,[StringComparison]::Ordinal);if($editStart-lt0-or$fitOut-lt$editStart-or$editReady-lt$fitOut-or$editClick-lt$editReady-or$editorReady-lt$editClick-or$amount150-lt$editorReady-or$updatePost-lt$amount150){throw 'Actual Costs edit sequencing contract is invalid'}
 $archiveStart=$fn.IndexOf("'archive dialog'",[StringComparison]::Ordinal);$rent=$fn.IndexOf("'Smoke Rent'",$archiveStart,[StringComparison]::Ordinal);$archiveReady=$fn.IndexOf('Archive Actual Cost selection did not stabilize.',$rent,[StringComparison]::Ordinal);$archiveClick=$fn.IndexOf("Invoke-AutomationIdButton `$Main 'ArchiveActualCostButton'",$archiveReady,[StringComparison]::Ordinal);if($archiveStart-lt0-or$rent-lt$archiveStart-or$archiveReady-lt$rent-or$archiveClick-lt$archiveReady){throw 'Actual Costs archive selection sequencing contract is invalid'}
 foreach($phase in 'create fit-out','create rent','create verification','totals','update','re-entry','archive dialog','archive cancel','archive confirm'){if(-not$fn.Contains("Invoke-ActualCostsPhase `$Main '$phase'")){throw "Actual Costs phase diagnostics missing: $phase"}}
 $all=Get-Content $path -Raw;$invoke=$all.IndexOf('Invoke-ActualCostsCrudSmoke $Main',[StringComparison]::Ordinal);$reentry=$all.IndexOf('BusinessProjectsCrud: re-entry after Actual Costs PASS',[StringComparison]::Ordinal);if($invoke-lt0-or$reentry-lt$invoke-or-not$all.Substring($invoke,$reentry-$invoke).Contains('Wait-BusinessProjectStatusReady')){throw 'missing semantic BusinessProjects re-entry after Actual Costs'}
}

Assert 'Actual Costs selections consume unambiguous event data' {
 $source=Get-Content (Join-Path $RepoRoot 'src/BusinessOS.Desktop/MainWindow.xaml.cs') -Raw
 foreach($spec in @(@('ActualCostsProjectSelector_SelectionChanged','ActualCostsList_SelectionChanged','OfType<BudgetProjectInfo>()','addedProjects.Length != 1','ActualCosts.SelectProjectAsync(project)','(sender as ComboBox)?.SelectedItem'),@('ActualCostsList_SelectionChanged','ActualCostsRefresh_Click','OfType<ActualCostItem>()','addedCosts.Length != 1','ActualCosts.SelectCostAsync(cost)','(sender as ListView)?.SelectedItem'))){$match=[regex]::Match($source,"(?s)private async void $($spec[0])\b.*?(?=\s*private .*?$($spec[1])\b)");if(-not$match.Success){throw "handler missing: $($spec[0])"};$handler=$match.Value;foreach($required in 'e.AddedItems',$spec[2],$spec[3],$spec[4]){if(-not$handler.Contains($required)){throw "$($spec[0]) missing $required"}};if($handler.Contains($spec[5])){throw "$($spec[0]) rereads sender selection"}}
}

Assert 'desktop Ready smoke implements complete semantic Budget Variance workflow' {
 $path=Join-Path $RepoRoot 'eng/smoke-test-desktop.ps1';$tokens=$null;$errors=$null;$ast=[System.Management.Automation.Language.Parser]::ParseFile($path,[ref]$tokens,[ref]$errors);if($errors.Count-ne0){throw "smoke parser errors: $($errors.Message-join'; ')"}
 foreach($name in 'Get-BudgetVarianceReadinessState','Get-BudgetVarianceSnapshotState','Write-BudgetVarianceTimeoutDiagnostics','Invoke-BudgetVariancePhase','Invoke-BudgetVarianceSmoke'){$definitions=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true));if($definitions.Count-ne1){throw "expected exactly one $name definition"}}
 $fn=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Invoke-BudgetVarianceSmoke'},$true))[0].Extent.Text
 foreach($required in 'BudgetVarianceSectionButton','BusinessOS Gym Smoke Updated','BusinessOS Budget Smoke Updated','Version 1','Version 2','RequiredConsecutiveSuccesses 3','BudgetVariance: version 1 PASS','BudgetVariance: version 2 PASS','BudgetVariance: re-entry PASS','CompaniesSectionButton','BusinessProjectsSectionButton'){if(-not$fn.Contains($required)){throw "Budget Variance smoke missing: $required"}}
 foreach($forbidden in 'IsOffscreen','BoundingRectangle','Test-Visible','Start-Sleep','Select-Object -First','Select-Object -Last','[0]'){if($fn.Contains($forbidden)){throw "Budget Variance smoke contains forbidden construct: $forbidden"}}
 foreach($name in 'Get-BudgetVarianceReadinessState','Get-BudgetVarianceSnapshotState'){$helper=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true))[0].Extent.Text;foreach($required in 'try','catch','$false'){if(-not$helper.Contains($required)){throw "$name is not exception-safe"}}}
 $sequence=@('BudgetVarianceSectionButton','Budget Variance project selector not ready.','BudgetVarianceProjectSelector','Budget Variance project selection failed.','BudgetVarianceBudgetSelector','Budget Variance budget selection failed.','Version 1','BudgetVariance: version 1 PASS','Version 2','BudgetVariance: version 2 PASS','CompaniesSectionButton','BudgetVarianceSectionButton','BudgetVariance: re-entry PASS');$offset=0;foreach($item in $sequence){$next=$fn.IndexOf($item,$offset,[StringComparison]::Ordinal);if($next-lt$offset){throw "Budget Variance sequence missing/out of order: $item"};$offset=$next+$item.Length}
 $all=$ast.Extent.Text;$invoke=$all.IndexOf('Invoke-BudgetVarianceSmoke $Main',[StringComparison]::Ordinal);$marker=$all.IndexOf('BusinessProjectsCrud: re-entry after Budget Variance PASS',$invoke,[StringComparison]::Ordinal);if($invoke-lt0-or$marker-lt$invoke-or-not$all.Substring($invoke,$marker-$invoke).Contains('Wait-BusinessProjectStatusReady')){throw 'missing stable BusinessProjects re-entry after Budget Variance'}
}

Assert 'Budget Variance selections consume unambiguous event data' {
 $source=Get-Content (Join-Path $RepoRoot 'src/BusinessOS.Desktop/MainWindow.xaml.cs') -Raw
 foreach($spec in @(@('BudgetVarianceProjectSelector_SelectionChanged','BudgetVarianceBudgetSelector_SelectionChanged','OfType<BudgetProjectInfo>()','addedProjects.Length != 1','Single()','BudgetVariance.SelectProjectAsync(project)'),@('BudgetVarianceBudgetSelector_SelectionChanged','BudgetVarianceVersionSelector_SelectionChanged','OfType<BudgetVarianceBudgetItem>()','addedBudgets.Length != 1','Single()','BudgetVariance.SelectBudgetAsync(budget)'),@('BudgetVarianceVersionSelector_SelectionChanged','BudgetVarianceRefresh_Click','OfType<BudgetVarianceVersionItem>()','addedVersions.Length != 1','Single()','BudgetVariance.SelectVersionAsync(version)'))){$match=[regex]::Match($source,"(?s)private async void $($spec[0])\b.*?(?=\s*private .*?$($spec[1])\b)");if(-not$match.Success){throw "handler missing: $($spec[0])"};$handler=$match.Value;foreach($required in 'e.AddedItems',$spec[2],$spec[3],$spec[4],$spec[5]){if(-not$handler.Contains($required)){throw "$($spec[0]) missing $required"}};if($handler.Contains('sender.SelectedItem')-or$handler.Contains('(sender as ComboBox)?.SelectedItem')){throw "$($spec[0]) rereads sender selection"}}
}
...
Assert 'desktop Ready smoke implements complete semantic Cost Cash Flow workflow' {
 $path=Join-Path $RepoRoot 'eng/smoke-test-desktop.ps1';$tokens=$null;$errors=$null;$ast=[System.Management.Automation.Language.Parser]::ParseFile($path,[ref]$tokens,[ref]$errors);if($errors.Count){throw 'desktop smoke does not parse'}
 foreach($name in 'Get-CostCashFlowReadinessState','Get-CostCashFlowMonthRows','Get-CostCashFlowSnapshotState','Write-CostCashFlowTimeoutDiagnostics','Invoke-CostCashFlowPhase','Invoke-CostCashFlowSmoke'){if(@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true)).Count-ne1){throw "missing unique $name"}}
 $fn=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Invoke-CostCashFlowSmoke'},$true))[0].Extent.Text
 foreach($required in 'CostCashFlowSectionButton','CostCashFlowProjectSelector','Select-ComboBoxExactSemanticItem','BusinessOS Gym Smoke Updated','RequiredConsecutiveSuccesses 3','Get-CostCashFlowSnapshotState','CostCashFlow: snapshot PASS','CompaniesSectionButton','CostCashFlow: re-entry PASS','BusinessProjectsSectionButton'){if(-not$fn.Contains($required)){throw "Cost Cash Flow smoke missing: $required"}}
 foreach($name in 'Invoke-CostCashFlowSmoke','Get-CostCashFlowReadinessState','Get-CostCashFlowMonthRows','Get-CostCashFlowSnapshotState'){$helper=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true))[0].Extent.Text;foreach($forbidden in 'IsOffscreen','BoundingRectangle','Test-Visible','Start-Sleep','Select-Object -First','Select-Object -Last','[0]'){if($helper.Contains($forbidden)){throw "$name contains forbidden construct: $forbidden"}}}
 foreach($name in 'Get-CostCashFlowReadinessState','Get-CostCashFlowMonthRows','Get-CostCashFlowSnapshotState'){$helper=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true))[0].Extent.Text;foreach($required in 'try','catch','$false'){if(-not$helper.Contains($required)){throw "$name is not exception-safe"}}}
 $all=$ast.Extent.Text;foreach($marker in 'CostCashFlow: snapshot PASS','CostCashFlow: re-entry PASS','BusinessProjectsCrud: re-entry after Cost Cash Flow PASS'){if(-not$all.Contains($marker)){throw "missing marker: $marker"}}
 $sequence=@('Invoke-CostCashFlowSmoke $Main','Wait-BusinessProjectStatusReady','BusinessProjects section did not restore the expected Draft project after Cost Cash Flow.','BusinessProjectsCrud: re-entry after Cost Cash Flow PASS','ChangeBusinessProjectStatusButton');$offset=0;foreach($item in $sequence){$next=$all.IndexOf($item,$offset,[StringComparison]::Ordinal);if($next-lt$offset){throw "Cost Cash Flow Ready sequence missing/out of order: $item"};$offset=$next+$item.Length}
}
Assert 'Cost Cash Flow project selection consumes unambiguous event data' {$source=Get-Content (Join-Path $RepoRoot 'src/BusinessOS.Desktop/MainWindow.xaml.cs') -Raw;$match=[regex]::Match($source,'(?s)private async void CostCashFlowProjectSelector_SelectionChanged\b.*?(?=\s*private async void CostCashFlowRefresh_Click\b)');if(-not$match.Success){throw 'Cost Cash Flow handler missing'};$handler=$match.Value;foreach($required in 'e.AddedItems','OfType<BudgetProjectInfo>()','ToArray()','addedProjects.Length != 1','Single()','CostCashFlow.SelectProjectAsync(project)'){if(-not$handler.Contains($required)){throw "Cost Cash Flow handler missing $required"}};foreach($forbidden in 'sender.SelectedItem','(sender as ComboBox)?.SelectedItem'){if($handler.Contains($forbidden)){throw 'Cost Cash Flow handler rereads sender selection'}}}
Assert 'Forecast Costs selections consume unambiguous event data' {
 $source=Get-Content (Join-Path $RepoRoot 'src/BusinessOS.Desktop/MainWindow.xaml.cs') -Raw
 foreach($spec in @(@('ForecastCostsProjectSelector_SelectionChanged','ForecastCostsList_SelectionChanged','OfType<BudgetProjectInfo>()','addedProjects.Length != 1','Single()','ForecastCosts.SelectProjectAsync(project)'),@('ForecastCostsList_SelectionChanged','ForecastCostsRefresh_Click','OfType<ForecastCostItem>()','addedForecasts.Length != 1','Single()','ForecastCosts.SelectForecastAsync(forecast)'))){$match=[regex]::Match($source,"(?s)private async void $($spec[0])\b.*?(?=\s*private .*?$($spec[1])\b)");if(-not$match.Success){throw "handler missing: $($spec[0])"};$handler=$match.Value;foreach($required in 'e.AddedItems',$spec[2],$spec[3],$spec[4],$spec[5]){if(-not$handler.Contains($required)){throw "$($spec[0]) missing $required"}};foreach($forbidden in 'sender.SelectedItem','(sender as ComboBox)?.SelectedItem','(sender as ListView)?.SelectedItem'){if($handler.Contains($forbidden)){throw "$($spec[0]) rereads sender selection"}}}
}

Assert 'desktop Ready smoke implements complete semantic Forecast Costs workflow' {
 $path=Join-Path $RepoRoot 'eng/smoke-test-desktop.ps1';$tokens=$null;$errors=$null;$ast=[System.Management.Automation.Language.Parser]::ParseFile($path,[ref]$tokens,[ref]$errors);if($errors.Count){throw 'desktop smoke does not parse'}
 foreach($name in 'Get-ForecastCostsReadinessState','Get-ForecastCostRowState','Test-ForecastCostEditorReady','Test-ForecastCostEditorClosed','Write-ForecastCostsTimeoutDiagnostics','Invoke-ForecastCostsPhase','Invoke-ForecastCostsCrudSmoke'){if(@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true)).Count-ne1){throw "missing unique $name"}}
 $fn=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Invoke-ForecastCostsCrudSmoke'},$true))[0].Extent.Text
 foreach($required in 'BusinessOS Gym Smoke Updated','ForecastCostsSectionButton','Select-ComboBoxExactSemanticItem','Get-ForecastCostRowState','RequiredConsecutiveSuccesses 3','Smoke Equipment Forecast','Smoke Utilities Forecast','ForecastCostCapexTotal','ForecastCostOpexTotal','ForecastCostTotal','ForecastCostsCrud: readiness PASS','ForecastCostsCrud: create PASS','ForecastCostsCrud: totals PASS','ForecastCostsCrud: update PASS','ForecastCostsCrud: re-entry PASS','ForecastCostsCrud: archive cancel PASS','ForecastCostsCrud: archive PASS'){if(-not$fn.Contains($required)){throw "Forecast Costs smoke missing: $required"}}
 foreach($forbidden in 'Select-Object -First','Select-Object -Last','Start-Sleep','IsOffscreen','BoundingRectangle','Test-Visible','[0]'){if($fn.Contains($forbidden)){throw "forbidden Forecast Costs smoke construct: $forbidden"}}
 foreach($helperName in 'Test-ForecastCostEditorReady','Test-ForecastCostEditorClosed','Get-ForecastCostsReadinessState'){$helper=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$helperName},$true))[0].Extent.Text;foreach($forbidden in 'IsOffscreen','BoundingRectangle','Test-Visible'){if($helper.Contains($forbidden)){throw "$helperName contains forbidden correctness dependency: $forbidden"}}}
 foreach($helperName in 'Test-ForecastCostEditorReady','Test-ForecastCostEditorClosed'){$helper=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$helperName},$true))[0].Extent.Text;foreach($required in 'try','catch','$false'){if(-not$helper.Contains($required)){throw "$helperName is not transient-exception safe"}}}
 $sequence=@("'create equipment'",'AddForecastCostButton','Test-ForecastCostEditorReady','SaveForecastCostButton','post-save semantic wait',"'create utilities'",'AddForecastCostButton',"'update'",'Smoke Equipment Forecast','Edit Forecast Cost selection did not stabilize','EditForecastCostButton','Edit Forecast Cost editor readiness failed',"'75'",'SaveForecastCostButton',"'re-entry'",'CompaniesSectionButton','ForecastCostsSectionButton','re-entry selector not ready','Select-ComboBoxExactSemanticItem',"'archive dialog'",'Smoke Utilities Forecast','Archive Forecast Cost selection did not stabilize','ArchiveForecastCostButton',"'archive cancel'",'CancelArchiveForecastCostButton',"'archive confirm'",'ConfirmArchiveForecastCostButton','ForecastCostsCrud: archive PASS');$offset=0;foreach($item in $sequence){$next=$fn.IndexOf($item,$offset,[StringComparison]::Ordinal);if($next-lt$offset){throw "Forecast Costs sequence missing/out of order: $item"};$offset=$next+$item.Length}
 foreach($phase in 'readiness','create equipment','create utilities','create verification','totals','update','re-entry','archive dialog','archive cancel','archive confirm'){if(-not$fn.Contains("Invoke-ForecastCostsPhase `$Main '$phase'")){throw "Forecast Costs phase diagnostics missing: $phase"}}
 $all=$ast.Extent.Text;$invoke=$all.IndexOf('Invoke-ForecastCostsCrudSmoke $Main',[StringComparison]::Ordinal);$marker=$all.IndexOf('BusinessProjectsCrud: re-entry after Forecast Costs PASS',$invoke,[StringComparison]::Ordinal);if($invoke-lt0-or$marker-lt$invoke-or-not$all.Substring($invoke,$marker-$invoke).Contains('Wait-BusinessProjectStatusReady')){throw 'missing stable BusinessProjects re-entry after Forecast Costs'}
}

Assert 'desktop Ready smoke implements complete semantic Budget Forecast workflow' {
 $path=Join-Path $RepoRoot 'eng/smoke-test-desktop.ps1';$tokens=$null;$errors=$null;$ast=[System.Management.Automation.Language.Parser]::ParseFile($path,[ref]$tokens,[ref]$errors);if($errors.Count-ne0){throw "smoke parser errors: $($errors.Message-join'; ')"}
 foreach($name in 'Get-BudgetForecastReadinessState','Get-BudgetForecastMetricState','Get-BudgetForecastSnapshotState','Write-BudgetForecastTimeoutDiagnostics','Invoke-BudgetForecastPhase','Invoke-BudgetForecastSmoke'){$definitions=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true));if($definitions.Count-ne1){throw "expected exactly one $name definition"}}
 $fn=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Invoke-BudgetForecastSmoke'},$true))[0].Extent.Text
 foreach($required in 'BudgetForecastSectionButton','BusinessOS Gym Smoke Updated','BusinessOS Budget Smoke Updated','Version 1','Version 2','RequiredConsecutiveSuccesses 3','BudgetForecast: version 1 PASS','BudgetForecast: version 2 PASS','BudgetForecast: re-entry PASS','CompaniesSectionButton','BusinessProjectsSectionButton'){if(-not$fn.Contains($required)){throw "Budget Forecast smoke missing: $required"}}
 $sequence=@('BudgetForecastSectionButton',"'readiness'",'Budget Forecast project selector not ready.','BudgetForecastProjectSelector','ProjectSemantic.IsExpected-and$r.BudgetReady','BudgetForecastBudgetSelector','BudgetSemantic.IsExpected-and$r.VersionReady',"'version 1'","'Version 1'",'VersionSemantic.IsExpected','BudgetForecast: version 1 PASS',"'version 2'","'Version 2'",'VersionSemantic.IsExpected','BudgetForecast: version 2 PASS',"'re-entry'",'CompaniesSectionButton','Budget Forecast navigation did not restabilize.','BudgetForecastSectionButton','re-entry project readiness failed.','BudgetForecastProjectSelector','ProjectSemantic.IsExpected-and$r.BudgetReady','BudgetForecastBudgetSelector','ProjectSemantic.IsExpected-and$r.BudgetSemantic.IsExpected-and$r.VersionReady',"'Version 2'",'VersionSemantic.IsExpected','snapshot.Matches','BudgetForecast: re-entry PASS','BusinessProjectsSectionButton');$offset=0;foreach($item in $sequence){$next=$fn.IndexOf($item,$offset,[StringComparison]::Ordinal);if($next-lt$offset){throw "Budget Forecast sequence missing/out of order: $item"};$offset=$next+$item.Length}
 foreach($name in 'Invoke-BudgetForecastSmoke','Get-BudgetForecastReadinessState','Get-BudgetForecastMetricState','Get-BudgetForecastSnapshotState'){$helper=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true))[0].Extent.Text;foreach($forbidden in 'IsOffscreen','BoundingRectangle','Test-Visible','Start-Sleep','Select-Object -First','Select-Object -Last','[0]'){if($helper.Contains($forbidden)){throw "$name contains forbidden construct: $forbidden"}}}
 foreach($name in 'Get-BudgetForecastReadinessState','Get-BudgetForecastMetricState','Get-BudgetForecastSnapshotState'){$helper=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true))[0].Extent.Text;foreach($required in 'try','catch','$false'){if(-not$helper.Contains($required)){throw "$name is not exception-safe"}}}
 $all=$ast.Extent.Text;$invoke=$all.IndexOf('Invoke-BudgetForecastSmoke $Main',[StringComparison]::Ordinal);$marker=$all.IndexOf('BusinessProjectsCrud: re-entry after Budget Forecast PASS',$invoke,[StringComparison]::Ordinal);if($invoke-lt0-or$marker-lt$invoke-or-not$all.Substring($invoke,$marker-$invoke).Contains('Wait-BusinessProjectStatusReady')){throw 'missing stable BusinessProjects re-entry after Budget Forecast'}
}
Assert 'Budget Forecast selections consume unambiguous event data' {
 $source=Get-Content (Join-Path $RepoRoot 'src/BusinessOS.Desktop/MainWindow.xaml.cs') -Raw
 foreach($spec in @(@('BudgetForecastProjectSelector_SelectionChanged','BudgetForecastBudgetSelector_SelectionChanged','OfType<BudgetProjectInfo>()','addedProjects.Length != 1','BudgetForecast.SelectProjectAsync(project)'),@('BudgetForecastBudgetSelector_SelectionChanged','BudgetForecastVersionSelector_SelectionChanged','OfType<BudgetForecastBudgetItem>()','addedBudgets.Length != 1','BudgetForecast.SelectBudgetAsync(budget)'),@('BudgetForecastVersionSelector_SelectionChanged','BudgetForecastRefresh_Click','OfType<BudgetForecastVersionItem>()','addedVersions.Length != 1','BudgetForecast.SelectVersionAsync(version)'))){$match=[regex]::Match($source,"(?s)private async void $($spec[0])\b.*?(?=\s*private .*?$($spec[1])\b)");if(-not$match.Success){throw "handler missing: $($spec[0])"};$handler=$match.Value;foreach($required in 'e.AddedItems',$spec[2],$spec[3],'ToArray()','Single()',$spec[4]){if(-not$handler.Contains($required)){throw "$($spec[0]) missing $required"}};if($handler.Contains('sender.SelectedItem')-or$handler.Contains('(sender as ComboBox)?.SelectedItem')){throw "$($spec[0]) rereads sender selection"}}
}

Assert 'Supplier Invoice posting dialog uses an automation-visible heading marker' {
 $path=Join-Path $RepoRoot 'src/BusinessOS.Desktop/MainWindow.xaml';[xml]$xaml=Get-Content -LiteralPath $path -Raw
 $markers=@($xaml.SelectNodes("//*[@AutomationProperties.AutomationId='PostSupplierInvoiceDialog']"));if($markers.Count-ne1){throw "expected exactly one PostSupplierInvoiceDialog marker, found $($markers.Count)"}
 $marker=$markers[0];if($marker.LocalName-ne'TextBlock'){throw "PostSupplierInvoiceDialog must mark an automation-visible TextBlock, not $($marker.LocalName)"};if($marker.GetAttribute('Text')-cne'Księgowanie faktury'){throw 'posting heading semantic name changed'}
 foreach($panelName in 'StackPanel','Grid','Panel','Canvas','RelativePanel'){if($marker.LocalName-eq$panelName){throw "PostSupplierInvoiceDialog cannot mark layout-only $panelName"}}
}

Assert 'desktop Ready smoke implements complete Supplier Invoice Posting workflow' {
 $path=Join-Path $RepoRoot 'eng/smoke-test-desktop.ps1';$tokens=$null;$errors=$null;$ast=[System.Management.Automation.Language.Parser]::ParseFile($path,[ref]$tokens,[ref]$errors);if($errors.Count){throw "desktop smoke does not parse: $($errors.Message-join'; ')"}
 $defs=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Invoke-SupplierInvoicePostingSmoke'},$true));if($defs.Count-ne1){throw 'missing unique Invoke-SupplierInvoicePostingSmoke'};$fn=$defs[0].Extent.Text
 foreach($required in 'SMOKE-INV-001','PostSupplierInvoiceButton','PostSupplierInvoiceDialog','PostSupplierInvoiceSourceSupplier','PostSupplierInvoiceSourceNumber','PostSupplierInvoiceSourceAmount','PostSupplierInvoiceSourceInvoiceDate','PostSupplierInvoiceKindSelector','Capex','ConfirmPostSupplierInvoiceButton','SupplierInvoicePosting: dialog PASS','SupplierInvoicePosting: post PASS','ActualCostsSectionButton','ActualCostsProjectSelector','Faktura SMOKE-INV-001','SupplierInvoicePosting: actual cost PASS','CompaniesSectionButton','SupplierInvoicesSectionButton','SupplierInvoicesProjectSelector','SupplierInvoicePosting: re-entry PASS','BusinessOS Gym Smoke Updated','RequiredConsecutiveSuccesses 3'){if(-not$fn.Contains($required)){throw "posting smoke missing $required"}}
 foreach($forbidden in 'IsOffscreen','BoundingRectangle','Test-Visible','Start-Sleep','Select-Object -First','Select-Object -Last','[0]'){if($fn.Contains($forbidden)){throw "posting smoke contains forbidden $forbidden"}}
 if(-not$fn.Contains("`$dialogMarker=Get-AutomationIdElement `$Main 'PostSupplierInvoiceDialog'")){throw 'posting dialog marker is not used as a Main-rooted sentinel'}
 foreach($id in 'PostSupplierInvoiceSourceSupplier','PostSupplierInvoiceSourceNumber','PostSupplierInvoiceSourceAmount','PostSupplierInvoiceSourceInvoiceDate','PostSupplierInvoiceKindSelector','CancelPostSupplierInvoiceButton','ConfirmPostSupplierInvoiceButton'){if(-not$fn.Contains("Get-AutomationIdElement `$Main '$id'")){throw "posting control is not resolved from Main: $id"};if($fn.Contains("Get-AutomationIdElement `$dialogMarker '$id'")){throw "posting control incorrectly depends on the dialog marker: $id"}}
 $sequence=@('PostSupplierInvoiceButton','SupplierInvoicePosting: dialog PASS','PostSupplierInvoiceKindSelector','Capex','ConfirmPostSupplierInvoiceButton','SupplierInvoicePosting: post PASS','ActualCostsSectionButton','Faktura SMOKE-INV-001','SupplierInvoicePosting: actual cost PASS','CompaniesSectionButton','SupplierInvoicesSectionButton','SupplierInvoicesProjectSelector','SupplierInvoicePosting: re-entry PASS');$offset=0;foreach($item in $sequence){$next=$fn.IndexOf($item,$offset,[StringComparison]::Ordinal);if($next-lt$offset){throw "posting sequence missing/out of order: $item"};$offset=$next+$item.Length}
 $all=$ast.Extent.Text;foreach($markerText in 'SupplierInvoicePosting: dialog PASS','SupplierInvoicePosting: post PASS','SupplierInvoicePosting: actual cost PASS','SupplierInvoicePosting: re-entry PASS'){if(([regex]::Matches($all,[regex]::Escape($markerText))).Count-ne1){throw "posting marker not unique: $markerText"}}
 $crud=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Invoke-SupplierInvoicesCrudSmoke'},$true)) | ForEach-Object {$_.Extent.Text};$offset=0;foreach($item in 'SupplierInvoicesCrud: archive PASS','Invoke-SupplierInvoicePostingSmoke $Main','BusinessProjectsSectionButton'){$next=$crud.IndexOf($item,$offset,[StringComparison]::Ordinal);if($next-lt$offset){throw "posting call chain missing/out of order: $item"};$offset=$next+$item.Length}
 $postStateDefs=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Get-SupplierInvoicePostingPostState'},$true));if($postStateDefs.Count-ne1){throw 'missing unique Get-SupplierInvoicePostingPostState'};$postState=$postStateDefs[0].Extent.Text
 $totalDefs=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Test-InvariantLabeledDecimal'},$true));if($totalDefs.Count-ne1){throw 'missing unique Test-InvariantLabeledDecimal'};$totalHelper=$totalDefs[0].Extent.Text
 foreach($required in '[regex]::Escape($Label)',': (?<Amount>[0-9]+(?:\.[0-9]+)?)$','[decimal]$amount = 0','[decimal]::TryParse','NumberStyles]::AllowDecimalPoint','CultureInfo]::InvariantCulture','$amount-eq$Expected'){if(-not$totalHelper.Contains($required)){throw "invariant labeled decimal helper missing $required"}}
 $totalFunction=[scriptblock]::Create($totalHelper);. $totalFunction
 foreach($case in @(@('Total: 135','Total',135),@('Total: 135.0','Total',135),@('Total: 135.00','Total',135),@('CAPEX: 285','CAPEX',285),@('CAPEX: 285.0','CAPEX',285),@('CAPEX: 285.00','CAPEX',285),@('OPEX: 0','OPEX',0),@('OPEX: 0.0','OPEX',0),@('OPEX: 0.00','OPEX',0))){if(-not(Test-InvariantLabeledDecimal $case[0] $case[1] ([decimal]$case[2]))){throw "semantic labeled decimal rejected: $($case[0])"}}
 foreach($case in @(@('Total: 135.01','Total',135),@('Total: 1135','Total',135),@('Total: -135','Total',135),@('Total: nope','Total',135),@('Subtotal: 135','Total',135),@('Total: 135 extra','Total',135),@('CAPEX: 285 extra','CAPEX',285),@('CAPEX: 1285','CAPEX',285),@('OPEX: 1','OPEX',0),@('','Total',135),@('Total: ','Total',135))){if(Test-InvariantLabeledDecimal $case[0] $case[1] ([decimal]$case[2])){throw "semantic labeled decimal accepted invalid input: $($case[0])"}}
 foreach($required in 'LogicalRows.Count','LogicalRowNames.Count','LogicalRowNames','ExpectedInvoiceRow','-ceq','Zaksięgowana','PostButtonFound','EditButtonFound','ArchiveButtonFound',"Test-InvariantLabeledDecimal `$totalName 'Total' ([decimal]135)",'Faktura została zaksięgowana jako koszt.','CompaniesButtonEnabled','RecoveryButtonEnabled','DialogMarkerFound','DialogMarkerName','DialogMarkerIsOffscreen'){if(-not$postState.Contains($required)){throw "posting post-state helper missing $required"}}
 $matchesStart=$postState.IndexOf('$matches=',[StringComparison]::Ordinal);$matchesEnd=$postState.IndexOf('[pscustomobject]', $matchesStart,[StringComparison]::Ordinal);$correctness=$postState.Substring($matchesStart,$matchesEnd-$matchesStart);foreach($forbidden in 'PostSupplierInvoiceDialog','DialogMarker','IsOffscreen','BoundingRectangle','Test-Visible','Start-Sleep'){if($correctness.Contains($forbidden)){throw "posting Matches correctness contains forbidden $forbidden"}}
 foreach($forbidden in "-ceq'Total: 135'","-eq'Total: 135'","-match'135'"){if($correctness.Contains($forbidden)){throw "posting Matches correctness contains brittle total oracle $forbidden"}}
 $confirmStart=$fn.IndexOf("Invoke-SupplierInvoicePostingPhase `$Main 'posting confirm'",[StringComparison]::Ordinal);$confirmEnd=$fn.IndexOf('SupplierInvoicePosting: post PASS',$confirmStart,[StringComparison]::Ordinal);if($confirmStart-lt0-or$confirmEnd-lt$confirmStart){throw 'posting confirm diagnostic phase missing'};$confirmBody=$fn.Substring($confirmStart,$confirmEnd-$confirmStart);foreach($required in 'ConfirmPostSupplierInvoiceButton','Get-SupplierInvoicePostingPostState','$expectedInvoiceRow','.Matches','RequiredConsecutiveSuccesses 3'){if(-not$confirmBody.Contains($required)){throw "posting confirm oracle missing $required"}};if($confirmBody.Contains("$null-eq(Get-AutomationIdElement `$Main 'PostSupplierInvoiceDialog')")){throw 'post-confirm oracle still requires dialog marker absence'}
 $actualStart=$fn.IndexOf("Invoke-SupplierInvoicePostingPhase `$Main 'actual cost verification'",[StringComparison]::Ordinal);$actualEnd=$fn.IndexOf('SupplierInvoicePosting: actual cost PASS',$actualStart,[StringComparison]::Ordinal);$actualBody=$fn.Substring($actualStart,$actualEnd-$actualStart);foreach($required in "Get-ActualCostRowState `$Main 'Faktura SMOKE-INV-001' 135 'Capex'","ActualCostCapexTotal').Current.Name 'CAPEX' ([decimal]285)","ActualCostOpexTotal').Current.Name 'OPEX' ([decimal]0)","ActualCostTotal').Current.Name 'Total' ([decimal]285)"){if(-not$actualBody.Contains($required)){throw "posting actual cost semantic oracle missing $required"}};foreach($forbidden in "-eq'CAPEX: 285'","-ceq'CAPEX: 285'","-eq'OPEX: 0'","-ceq'OPEX: 0'","-eq'Total: 285'","-ceq'Total: 285'","-match'285'"){if($actualBody.Contains($forbidden)){throw "posting actual cost contains brittle total oracle $forbidden"}}
 foreach($phase in @(@('actual cost verification','SupplierInvoicePosting: actual cost PASS'),@('posting re-entry','SupplierInvoicePosting: re-entry PASS'))){$start=$fn.IndexOf("Invoke-SupplierInvoicePostingPhase `$Main '$($phase[0])'",[StringComparison]::Ordinal);$end=$fn.IndexOf($phase[1],$start,[StringComparison]::Ordinal);if($start-lt0-or$end-lt$start){throw "posting diagnostic wrapper missing phase: $($phase[0])"}}
 $reentryStart=$fn.IndexOf("Invoke-SupplierInvoicePostingPhase `$Main 'posting re-entry'",[StringComparison]::Ordinal);$reentryEnd=$fn.IndexOf('SupplierInvoicePosting: re-entry PASS',$reentryStart,[StringComparison]::Ordinal);$reentryBody=$fn.Substring($reentryStart,$reentryEnd-$reentryStart);foreach($required in 'LogicalRows.Count','LogicalRowNames.Count','LogicalRowNames','expectedInvoiceRow','-ceq','Zaksięgowana','PostSupplierInvoiceButton','EditSupplierInvoiceButton','ArchiveSupplierInvoiceButton','Test-InvariantLabeledDecimal','SupplierInvoicesTotal',"'Total'",'([decimal]135)'){if(-not$reentryBody.Contains($required)){throw "posting re-entry exact oracle missing $required"}}
 $diagnosticDefs=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Write-SupplierInvoicePostingTimeoutDiagnostics'},$true));if($diagnosticDefs.Count-ne1){throw 'missing unique posting diagnostics'};$postingDiagnostics=$diagnosticDefs[0].Extent.Text;foreach($required in 'expected project','expected semantic invoice row','SupplierInvoicesOperationMessage','PostSupplierInvoiceDialog','IsOffscreen','diagnostic only','logical invoice row count','logical SemanticName','exact invoice row','RuntimeId','exact invoice descendant Name','Zaksięgowana','PostSupplierInvoiceButton','EditSupplierInvoiceButton','ArchiveSupplierInvoiceButton','SupplierInvoicesTotal','CompaniesSectionButton','OpenRecoveryFromMainButton','ActualCostsProjectSelector','ActualCostsList','ActualCostCapexTotal','ActualCostOpexTotal','ActualCostTotal','Write-SmokeDiagnosticsToHost'){if(-not$postingDiagnostics.Contains($required)){throw "posting diagnostics missing $required"}}
 $readyInvoke=$all.IndexOf('Invoke-SupplierInvoicesCrudSmoke $Main',[StringComparison]::Ordinal);$wait=$all.IndexOf('Wait-BusinessProjectStatusReady',$readyInvoke,[StringComparison]::Ordinal);$reentry=$all.IndexOf('BusinessProjectsCrud: re-entry after Supplier Invoices PASS',$wait,[StringComparison]::Ordinal);$status=$all.IndexOf('ChangeBusinessProjectStatusButton',$reentry,[StringComparison]::Ordinal);if($readyInvoke-lt0-or$wait-lt$readyInvoke-or$reentry-lt$wait-or$status-lt$reentry){throw 'posting Ready flow is not monotonic through BusinessProjects status transition'}
 $viewModel=Get-Content (Join-Path $RepoRoot 'src/BusinessOS.Desktop/ViewModels/SupplierInvoicesViewModel.cs') -Raw;if($viewModel-match 'public\s+DateOnly\s+PostingInvoiceDate'){throw 'PostingInvoiceDate presentation must not expose DateOnly'};foreach($required in 'PostingInvoiceDate','yyyy-MM-dd','CultureInfo.InvariantCulture'){if(-not$viewModel.Contains($required)){throw "invariant PostingInvoiceDate contract missing: $required"}}
 foreach($contract in 'ISupplierInvoicePostingService','ISupplierInvoicePostingStore','PostedActualCostId','PostedAtUtc','MarkPosted','SupplierInvoiceOperationStatus.Posted','AddSupplierInvoicePosting'){$found=Get-ChildItem (Join-Path $RepoRoot 'src') -Recurse -File | Where-Object Extension -in '.cs','.xaml' | Select-String -SimpleMatch $contract -Quiet;if(-not$found){throw "posting source contract missing: $contract"}}
}

if($script:Failures -gt 0){exit 1}

Assert 'desktop Ready smoke implements complete semantic Supplier Invoices workflow' {
 $path=Join-Path $RepoRoot 'eng/smoke-test-desktop.ps1';$tokens=$null;$errors=$null;$ast=[System.Management.Automation.Language.Parser]::ParseFile($path,[ref]$tokens,[ref]$errors);if($errors.Count){throw "desktop smoke does not parse: $($errors.Message-join'; ')"}
 foreach($name in 'Get-SupplierInvoicesReadinessState','Get-SupplierInvoiceRowState','Test-SupplierInvoiceEditorReady','Test-SupplierInvoiceEditorClosed','Write-SupplierInvoicesTimeoutDiagnostics','Invoke-SupplierInvoicesPhase','Invoke-SupplierInvoicesCrudSmoke'){if(@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true)).Count-ne1){throw "missing unique $name"}}
 $calendarDefinitions=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Set-AutomationCalendarDate'},$true));if($calendarDefinitions.Count-ne1){throw 'expected exactly one dedicated Set-AutomationCalendarDate helper'}
 $calendarHelper=$calendarDefinitions | ForEach-Object {$_.Extent.Text}
 foreach($required in 'InvokePattern','RootElement','CalendarView','MonthViewScrollViewer','HeaderButton','PreviousButton','NextButton','GridItemPattern','Current.Row','Current.Column','RuntimeId','SelectionItemPattern','GetDaysInMonth','daysInTargetMonth','completeRuns.Count -ne 1','targetItems.Count -ne 1','RequiredConsecutiveSuccesses 3'){if(-not$calendarHelper.Contains($required)){throw "CalendarDatePicker helper semantic contract missing: $required"}}
 foreach($required in 'pickerProcessId','monthViewsByRuntimeId','semanticDayCount','isSemanticActiveCandidate','activeMonthViews.Count -ne 1','MonthViewScrollViewerCandidateCount','activeMonthViewCandidateCount','selectedMonthViewRuntimeId'){if(-not$calendarHelper.Contains($required)){throw "CalendarDatePicker active MonthView discovery contract missing: $required"}}
 foreach($forbidden in 'ValuePattern]::Pattern).SetValue','IsOffscreen','BoundingRectangle','Test-Visible','Start-Sleep','Select-Object -First','Select-Object -Last','[0]','YearViewScrollViewer','DecadeViewScrollViewer','beforeRuntimeIds'){if($calendarHelper.Contains($forbidden)){throw "CalendarDatePicker helper contains forbidden correctness construct: $forbidden"}}
 if($calendarHelper-match '(?s)DateTime\]::TryParse\([^\)]*day\.Current\.Name'){throw 'CalendarViewDayItem.Current.Name must be interpreted only as a day ordinal, never as a full DateTime'}
 $fn=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Invoke-SupplierInvoicesCrudSmoke'},$true)) | ForEach-Object {$_.Extent.Text}
 foreach($required in 'BusinessOS Gym Smoke Updated','SupplierInvoicesSectionButton','SupplierInvoicesProjectSelector','Select-ComboBoxExactSemanticItem','RequiredConsecutiveSuccesses 3','Smoke Equipment Vendor','SMOKE-INV-001','Smoke Utilities Vendor','SMOKE-UTIL-001','2026-01-10','2026-02-10','2026-01-15','2026-01-31','SupplierInvoicesCrud: readiness PASS','SupplierInvoicesCrud: create PASS','SupplierInvoicesCrud: totals PASS','SupplierInvoicesCrud: update PASS','SupplierInvoicesCrud: re-entry PASS','SupplierInvoicesCrud: archive cancel PASS','SupplierInvoicesCrud: archive PASS'){if(-not$fn.Contains($required)){throw "Supplier Invoices smoke missing: $required"}}
 foreach($id in 'SupplierInvoiceInvoiceDateInput','SupplierInvoiceDueDateInput'){if(-not$fn.Contains("Set-AutomationCalendarDate `$Main '$id'")){throw "Supplier Invoices does not use the calendar helper for $id"};if($fn.Contains("Set-AutomationValue `$Main '$id'")){throw "Supplier Invoices incorrectly uses writable ValuePattern for $id"}}
 foreach($literal in '2026-01-10','2026-02-10','2026-01-15','2026-01-31'){if(([regex]::Matches($fn,[regex]::Escape("[DateTime]'$literal'"))).Count-ne1){throw "Supplier Invoices must calendar-select literal date exactly once: $literal"}}
 $editorReady=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Test-SupplierInvoiceEditorReady'},$true)) | ForEach-Object {$_.Extent.Text};foreach($required in 'SupplierInvoiceInvoiceDateInput','SupplierInvoiceDueDateInput','InvokePattern','TryGetCurrentPattern'){if(-not$editorReady.Contains($required)){throw "Supplier Invoice editor readiness lacks CalendarDatePicker capability: $required"}}
 $readiness=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq'Get-SupplierInvoicesReadinessState'},$true)) | ForEach-Object {$_.Extent.Text};foreach($required in 'SupplierInvoicesList','TreeScope]::Children','ControlType]::ListItem','LogicalRows','LogicalRowNames','Current.Name'){if(-not$readiness.Contains($required)){throw "Supplier Invoices ordered list-state oracle missing: $required"}};if($readiness.Contains('Sort-Object')){throw 'Supplier Invoices ordered list-state oracle must preserve UIA ListView child order'}
 foreach($expected in 'Supplier=Smoke Utilities Vendor | Invoice=SMOKE-UTIL-001 | Amount=30 PLN | InvoiceDate=2026-01-15 | DueDate=2026-01-31','Supplier=Smoke Equipment Vendor | Invoice=SMOKE-INV-001 | Amount=120 PLN | InvoiceDate=2026-01-10 | DueDate=2026-02-10','Supplier=Smoke Equipment Vendor | Invoice=SMOKE-INV-001 | Amount=135 PLN | InvoiceDate=2026-01-10 | DueDate=2026-02-10'){if(-not$fn.Contains($expected)){throw "Supplier Invoices exact semantic order oracle missing row: $expected"}}
 foreach($phase in @(@('create invoice 2','SupplierInvoicesCrud: totals PASS','$initialOrderedRows'),@('edit invoice','SupplierInvoicesCrud: update PASS','$updatedOrderedRows'),@('re-entry','SupplierInvoicesCrud: re-entry PASS','$updatedOrderedRows'),@('archive dialog','SupplierInvoicesCrud: archive cancel PASS','$updatedOrderedRows'),@('archive confirm','SupplierInvoicesCrud: archive PASS','$archivedOrderedRows'))){$start=$fn.IndexOf("Invoke-SupplierInvoicesPhase `$Main '$($phase[0])'",[StringComparison]::Ordinal);$end=$fn.IndexOf($phase[1],$start,[StringComparison]::Ordinal);if($start-lt0-or$end-lt$start){throw "Supplier Invoices ordered phase missing: $($phase[0])"};$body=$fn.Substring($start,$end-$start);foreach($required in 'LogicalRows.Count','LogicalRowNames','-join$orderedSeparator','-ceq',$phase[2]){if(-not$body.Contains($required)){throw "Supplier Invoices phase '$($phase[0])' does not enforce one exact ordered-list oracle: $required"}}}
 $initialUtil=$fn.IndexOf('Supplier=Smoke Utilities Vendor | Invoice=SMOKE-UTIL-001 | Amount=30 PLN',[StringComparison]::Ordinal);$initialEquipment=$fn.IndexOf('Supplier=Smoke Equipment Vendor | Invoice=SMOKE-INV-001 | Amount=120 PLN',$initialUtil,[StringComparison]::Ordinal);if($initialUtil-lt0-or$initialEquipment-lt$initialUtil){throw 'Supplier Invoices initial ordered oracle must place UTIL before INV'}
 foreach($name in 'Get-SupplierInvoicesReadinessState','Get-SupplierInvoiceRowState','Test-SupplierInvoiceEditorReady','Test-SupplierInvoiceEditorClosed','Invoke-SupplierInvoicesCrudSmoke'){$helper=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true)) | ForEach-Object {$_.Extent.Text};foreach($forbidden in 'IsOffscreen','BoundingRectangle','Test-Visible','Start-Sleep','Select-Object -First','Select-Object -Last','[0]'){if($helper.Contains($forbidden)){throw "$name contains forbidden construct: $forbidden"}}}
 foreach($name in 'Get-SupplierInvoicesReadinessState','Get-SupplierInvoiceRowState','Test-SupplierInvoiceEditorReady','Test-SupplierInvoiceEditorClosed'){$helper=@($ast.FindAll({param($n)$n-is[System.Management.Automation.Language.FunctionDefinitionAst]-and$n.Name-eq$name},$true)) | ForEach-Object {$_.Extent.Text};foreach($required in 'try','catch','$false'){if(-not$helper.Contains($required)){throw "$name is not transient-exception safe"}}}
 $sequence=@('SupplierInvoicesSectionButton','readiness','Select-ComboBoxExactSemanticItem','SupplierInvoicesCrud: readiness PASS','create invoice 1','SupplierInvoicesCrud: create PASS','create invoice 2','SupplierInvoicesCrud: totals PASS','edit invoice','SupplierInvoicesCrud: update PASS','CompaniesSectionButton','SupplierInvoicesSectionButton','Select-ComboBoxExactSemanticItem','SupplierInvoicesCrud: re-entry PASS','archive dialog','SupplierInvoicesCrud: archive cancel PASS','archive confirm','SupplierInvoicesCrud: archive PASS','BusinessProjectsSectionButton');$offset=0;foreach($item in $sequence){$next=$fn.IndexOf($item,$offset,[StringComparison]::Ordinal);if($next-lt$offset){throw "Supplier Invoices sequence missing/out of order: $item"};$offset=$next+$item.Length}
 $all=$ast.Extent.Text;$invoke=$all.IndexOf('Invoke-SupplierInvoicesCrudSmoke $Main',[StringComparison]::Ordinal);$wait=$all.IndexOf('Wait-BusinessProjectStatusReady',$invoke,[StringComparison]::Ordinal);$marker=$all.IndexOf('BusinessProjectsCrud: re-entry after Supplier Invoices PASS',$wait,[StringComparison]::Ordinal);$status=$all.IndexOf('ChangeBusinessProjectStatusButton',$marker,[StringComparison]::Ordinal);if($invoke-lt0-or$wait-lt$invoke-or$marker-lt$wait-or$status-lt$marker){throw 'Supplier Invoices BusinessProjects re-entry is not before status transition'}
}

Assert 'Supplier Invoices selections consume unambiguous event data' {
 $source=Get-Content (Join-Path $RepoRoot 'src/BusinessOS.Desktop/MainWindow.xaml.cs') -Raw
 foreach($spec in @(@('SupplierInvoicesProjectSelector_SelectionChanged','SupplierInvoicesList_SelectionChanged','OfType<BudgetProjectInfo>()','SupplierInvoices.SelectProjectAsync'),@('SupplierInvoicesList_SelectionChanged','SupplierInvoicesRefresh_Click','OfType<SupplierInvoiceItem>()','SupplierInvoices.SelectInvoiceAsync'))){$match=[regex]::Match($source,"(?s)private async void $($spec[0])\b.*?(?=\s*private .*?$($spec[1])\b)");if(-not$match.Success){throw "handler missing: $($spec[0])"};$handler=$match.Value;foreach($required in 'e.AddedItems',$spec[2],'ToArray()','Length == 1','Single()',$spec[3]){if(-not$handler.Contains($required)){throw "$($spec[0]) missing $required"}};foreach($forbidden in 'sender.SelectedItem','(sender as ComboBox)?.SelectedItem','(sender as ListView)?.SelectedItem'){if($handler.Contains($forbidden)){throw "$($spec[0]) rereads sender selection"}}}
}

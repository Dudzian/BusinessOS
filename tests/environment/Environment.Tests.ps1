...
Assert 'vulnerability scanner accepts report without frameworks' {
  $d=New-VulnerabilityScanFixture; try{ $logPath=Join-Path $d 'args.jsonl'; $fake=New-FakeDotnetProbe $d; $oldLog=$env:BUSINESSOS_FAKE_DOTNET_LOG; $oldMode=$env:BUSINESSOS_FAKE_DOTNET_MODE; $env:BUSINESSOS_FAKE_DOTNET_LOG=$logPath; $env:BUSINESSOS_FAKE_DOTNET_MODE='clean'; $r=Invoke-ExpectSuccess -File 'pwsh' -ArgumentList @('-NoLogo','-NoProfile','-NonInteractive','-File',(Join-Path $d 'eng/check-vulnerable-packages.ps1'),'-ProjectOrSolution',(Join-Path $d 'BusinessOS.sln'),'-DotnetExecutable',$fake) -WorkingDirectory $d; if($r.Combined -notmatch [regex]::Escape('No vulnerable NuGet packages were reported')){throw 'success output did not report clean vulnerability result'}; $artifact=Get-Content -LiteralPath (Join-Path $d '.cache/vulnerable-packages.json') -Raw|ConvertFrom-Json; if(@($artifact.reports).Count -ne 1){throw "expected one report, got $(@($artifact.reports).Count)"}; $projects=@(@($artifact.reports)[0].projects); if($projects.Count -le 0){throw 'artifact report did not contain projects'}; $project=$projects[0]; if([string]::IsNullOrWhiteSpace([string]$project.path)){throw 'artifact project path missing'}; if($null -ne $project.frameworks){throw 'clean report unexpectedly contained frameworks'}; if($r.Combined -match 'Vulnerable NuGet packages were reported'){throw 'scanner reported vulnerabilities for clean report'} } finally { $env:BUSINESSOS_FAKE_DOTNET_LOG=$oldLog; $env:BUSINESSOS_FAKE_DOTNET_MODE=$oldMode; Remove-Item -LiteralPath $d -Recurse -Force -ErrorAction SilentlyContinue } }

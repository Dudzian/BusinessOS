...
$cache=Join-Path $Root $Lock.nugetCache; try{New-Item -ItemType Directory -Force -Path $cache|Out-Null; $t=Join-Path $cache '.write-test'; 'x'|Set-Content $t; Remove-Item $t; $cw=$true}catch{$cw=$false}; Add-Check 'NuGet cache' 'writable' $cache $cw
